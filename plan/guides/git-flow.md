...
```text
main          ← прод: стабильно, редко, только проверенное
  ↑ PR
dev           ← рабочая сборка: сюда мержим завершённые задачи
  ↑ PR
app-12fr      ← одна продуктовая задача (issue #12, frontend)
pr16gitflow   ← одна практика (issue #16)
  ↑ опционально
app-12fr-btn  ← подзадача, если объём реально разъехался
```

| Ветка | Роль | Кто мержит куда |
|-------|------|-----------------|
| `main` | То, что «можно считать продуктом» | Только из `dev`, когда сознательно решили выкатить |
| `dev` | Интеграция всей текущей работы | Сюда — все завершённые `app-*` / `pr*` |
| `app-<n><код>` | Задача продукта | PR → `dev` |
| `pr<n><слово>` | Учебная практика | PR → `dev` |

Коды для `app`: `fr` frontend, `be` backend, `md` docs/plan, `ci` CI, `db` БД — при необходимости расширим.
...
git checkout dev
git pull origin dev
git checkout -b app-12fr   # или pr16gitflow
```
...
Имена веток **не обязаны** совпадать 1:1 с title issue, но номер issue в имени (`app-12fr`) сильно упрощает жизнь.
